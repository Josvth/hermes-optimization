...
from models.models import compute_overlap_matrix, compute_passes_fspl, compute_passes_elevation_angles, \
    ...
    compute_passes_energy_simplified_visibility, compute_passes_energy_maee
...
class VCMProblem(Problem):
    ...
    def __init__(self, instances_df, system_parameters, single_power=True, requirements=Requirements(), *args,
                 **kwargs):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        if np.sum(sel_pass) > 0:
            ...
            Ptx_dBm_array = np.array(list(compress(design_vector['power'], sel_pass)))
            Gtx_dBi_array = np.array(list(compress(design_vector['antenna'].repeat(self.N_passes), sel_pass)))
            B_Hz_array = np.array(list(compress(map(self.sys_param.B_Hz_list.__getitem__, design_vector['bandwidth']), sel_pass)))
            alpha_array = np.array(list(compress(map(self.sys_param.alpha_list.__getitem__, design_vector['rolloff'].repeat(self.N_passes)), sel_pass)))
            ...
            if self.single_power:
                Ptx_dBm_array[:] = Ptx_dBm_array[0]

            theta_rad_list = List(compress(self.theta_rad_list, sel_pass))

            linktime_s_array, f_throughput = compute_passes_throughput_visibility(tof_s_list, theta_rad_list, fspl_dB_list,
                                                                                 Ptx_dBm_array, Gtx_dBi_array,
