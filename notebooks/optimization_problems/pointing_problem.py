...
class PointingProblem(Problem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        g_overlap = 0  # No over-lap constraint
        g_minimum = 0  # Minimum throughput constraint
        g_rate = np.zeros(self.N_passes)  # Pointing rate constraint
        ...
        if num_pass > 0:
            ...
            f_pointing, g_rate[sel_pass] = pointing.compute_pointing_fg_passes(tof_s_list, theta_rad_list, phi_rad_list,
                                                                               Gtx_dBi, self.reqs.max_rate_rads)
        ...
        out["G"] = np.concatenate([g_overlap, np.array([g_minimum]), g_rate])
