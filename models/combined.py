...
@njit
def _compute_fg_extended(N_passes, t_sim_s, tof_s_list, fspl_dB_list, theta_rad_list, phi_rad_list,
                  x_pass, Ptx_dBm_array, Gtx_dBi, GT_dBK, B_Hz, alpha,
                  EsN0_req_dB_array, eta_bitsym_array, eta_maee_array, margin_dB,
                  min_throughput, max_throughput, max_latency, max_energy, max_pointing, max_rate_rads
                  ):
...
## EXPERIMENTAL
@njit
def __compute_fg(N_passes, t_sim_s, tof_s_list, fspl_dB_list, theta_rad_list, phi_rad_list,
                 x_pass, Ptx_dBm_array, Gtx_dBi, GT_dBK, B_Hz, alpha,
                 EsN0_req_dB_array, eta_bitsym_array, eta_maee_array, margin_dB,
                 min_throughput, max_throughput, max_latency, max_energy, max_pointing, max_rate_rads,
                 func_throughput, func_energy, func_pointing, func_latency):
    ...
    if np.sum(x_pass) > 0:
        ...
        # Throughput
        b_s_array, e_s_array, linktime_s_array, f_throughput, vcm_array = func_throughput(
            pass_inds, tof_s_list, fspl_dB_list,
            Ptx_dBm_array, Gtx_dBi, GT_dBK, B_Hz, alpha,
            EsN0_req_dB_array, eta_bitsym_array, margin_dB)
        ...
        # Energy
        eta_maee_array = eta_maee_array[vcm_array]
        f_energy = func_energy(linktime_s_array, Ptx_dBm_array, eta_maee_array)

        # Pointing
        f_pointing, rates_rads_array[x_pass] = func_pointing(pass_inds, tof_s_list, theta_rad_list, phi_rad_list, Gtx_dBi)

        # Latency
        if b_s_array.size > 0:
            f_latency = func_latency(b_s_array, e_s_array, t_sim_s)
